...
impl QueryRuntime {  
    ...
    fn is_temporary_error(e: &io::Error) -> bool {  
        matches!(  
            e.kind(),  
        ...
        loop {  
            ...
            match self.socket.poll_recv(cx, &mut buf) {  
                ...
                Poll::Ready(Err(e)) => {  
                    ...
                    warn!(  
                        "UDP receive failed in route query runtime (attempt {}): {:?}",   
                        self.consecutive_errors, e  
                    );  
                      
                    // Implement exponential backoff for consecutive errors  
                    const MAX_RETRY_ATTEMPTS: u32 = 5;  
                    if self.consecutive_errors < MAX_RETRY_ATTEMPTS && Self::is_temporary_error(&e) {  
                        let backoff_duration = Duration::from_millis(  
                            100 * (1 << self.consecutive_errors.min(6))  
                        );  
                        ...
                        // Too many errors or permanent error - fail gracefully  
                        error!(  
                            "Route query runtime failed after {} consecutive errors",   
